...
class FCMReceiverService {
  ...
  Stream<Map<String, dynamic>> get onRoomInvitation =>
  ...
  Future<void> initialize() async {
    try {
      ...
      debugPrint(
        'FCMReceiverService: User granted permission: ${settings.authorizationStatus}',
      );
      ...
      if (initialMessage != null) {
        debugPrint(
          "FCMReceiverService: App opened from terminated state with message",
        );
  ...
  Future<void> _handleForegroundMessage(RemoteMessage message) async {
    ...
    if (_isRoomInvitation(message)) {
      ...
      debugPrint(
        "FCMReceiverService: Room invitation received in foreground: $data",
      );
  ...
  Future<void> _handleBackgroundMessageTap(RemoteMessage message) async {
    ...
    if (_isRoomInvitation(message)) {
      final data = _extractInvitationData(message.data);
      debugPrint(
        "FCMReceiverService: Room invitation tapped from background: $data",
      );
  ...
  Future<void> _handleInitialMessage(RemoteMessage message) async {
    if (_isRoomInvitation(message)) {
      final data = _extractInvitationData(message.data);
      debugPrint(
        "FCMReceiverService: Initial message contains room invitation: $data",
      );
    ...
    if (message.data.containsKey('metadata')) {
      ...
      if (metadata is String) {
        try {
          ...
          return metadata.contains('roomInvitation') ||
              metadata.contains('type":"roomInvitation');
        } catch (_) {
          return false;
        }
  ...
  Map<String, dynamic> _extractInvitationData(Map<String, dynamic> data) {
    ...
    if (data.containsKey('metadata')) {
      ...
      if (rawMetadata is String) {
        try {
          ...
          debugPrint(
            "FCMReceiverService: Extracted metadata from string: $metadata",
          );
        } catch (e) {
      ...
      // Handle metadata as Map
        ...
        debugPrint(
          "FCMReceiverService: Using metadata map directly: $metadata",
        );
      }
    }
    ...
    final result = {
      ...
      'sender_name':
          metadata['sender'] ?? data['sender_name'] ?? 'Unknown Caller',
  ...
  bool _isInvitationValid(String timestampStr) {
    ...
    debugPrint(
      "FCMReceiverService: Invitation ${isValid ? 'is valid' : 'expired'}. "
  ...
  Future<void> _autoJoinChannel(Map<String, dynamic> data) async {
    try {
      ...
      if (success) {
        ...
        debugPrint(
          "FCMReceiverService: Successfully joined channel $channelId",
        );
      } else {
        ...
        if (!await _agoraService.initialize()) {
          debugPrint(
            "FCMReceiverService: Failed to re-initialize Agora engine",
          );
          return;
        }
        ...
        if (retrySuccess) {
          await _agoraService.muteLocalAudio(true);
          debugPrint(
            "FCMReceiverService: Successfully joined channel on retry",
          );
        } else {
