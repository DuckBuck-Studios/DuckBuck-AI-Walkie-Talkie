...
class BackgroundCallService {
  // Singleton instance
  static final BackgroundCallService _instance =
  ...
  static const MethodChannel _channel = MethodChannel(
    'com.example.duckbuck/background_call',
  );
  static const MethodChannel _navigationChannel = MethodChannel(
    'com.example.duckbuck/navigation',
  );
  ...
  Future<Map<String, dynamic>?> checkPendingNavigation() async {
    try {
      final Map<String, dynamic>? result = await _navigationChannel
          .invokeMapMethod('checkPendingNavigation');
      if (result != null && result.isNotEmpty) {
        debugPrint(
          'BackgroundCallService: Received pending navigation: $result',
        );
        return result;
      }
    } catch (e) {
      debugPrint(
        'BackgroundCallService: Error checking pending navigation - $e',
      );
    }
    return null;
  }
  ...
  void handleAppLifecycleState(
    ...
    switch (state) {
      ...
      case AppLifecycleState.paused:
        ...
        if (callProvider.callState == CallState.connected) {
          debugPrint(
            'App paused with active call - ensuring background service is running',
          );
        }
        break;
  ...
  Future<void> handlePendingCallNavigation(
    GlobalKey<NavigatorState> navigatorKey,
  ) async {
    try {
      ...
      if (callData != null &&
        ...
        debugPrint(
          'BackgroundCallService: Navigating to call screen from notification: $callData',
        );
      ...
      debugPrint(
        'BackgroundCallService: Error handling pending call navigation - $e',
      );
    }
  }
  ...
  Future<bool> startBackgroundService({
    ...
    try {
      ...
      _callStartTime = DateTime.now().subtract(
        Duration(seconds: initialDurationSeconds),
      );

      debugPrint(
        'BackgroundCallService: Starting with caller $callerName, avatar: $callerAvatar, uid: $senderUid',
      );
      ...
      debugPrint(
        'BackgroundCallService: Background service started with ${initialDurationSeconds}s initial duration',
      );
      return true;
    } catch (e) {
      debugPrint(
        'BackgroundCallService: Error starting background service - $e',
      );
      return false;
    }
  }
  ...
  void _startCallDurationTimer() {
    ...
    _callDurationTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (currentDuration % 60 == 0 && currentDuration > 0) {
        debugPrint(
          'BackgroundCallService: Call duration: ${_formatDuration(currentDuration)}',
        );
      }
    });
  }
  ...
  Future<void> stopBackgroundService({
    ...
    try {
      ...
      // Use the provided caller name or the stored one
      final displayCallerName = callerName ?? _callerName ?? "Unknown";

      if (Platform.isAndroid) {
        // Use native Android implementation
        if (showEndedNotification) {
          // Update call attributes with call ended flag and ensure caller name is set
          ...
          // Give a delay to show the notification
      ...
      debugPrint(
        'BackgroundCallService: Error stopping background service - $e',
      );
    }
  }
  ...
  Future<void> markCallEndedByRemote() async {
    debugPrint('BackgroundCallService: Call ended by remote user');

    try {
      // Use the stored caller name
      final displayCallerName = _callerName ?? "Unknown";

      if (Platform.isAndroid) {
        // First explicitly update with isCallEnded=true to show notification
        await _channel.invokeMethod('updateCallAttributes', {
          'callerName': displayCallerName,
          'callerAvatar': _callerAvatar,
          'senderUid': _senderUid,
        ...
        // Give a longer delay to ensure the notification is displayed properly
        await Future.delayed(const Duration(milliseconds: 1500));
      }

      // Then stop the service with the remote ended flag
      ...
      debugPrint(
        'BackgroundCallService: Error marking call as ended by remote - $e',
      );
      // Fallback to just stopping the service
      stopBackgroundService(showEndedNotification: true, remoteEnded: true);
    }
  }
